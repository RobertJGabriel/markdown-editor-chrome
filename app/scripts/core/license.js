...
let get = async () => {
	...
	return new Promise((resolve, reject) => {
		...
		const API =
			'https://rerjk6ho03.execute-api.us-east-1.amazonaws.com/production';
		AXIOS.get(`${API}/api/payments/user?email=${email}`)
			.then((response) => {
				...
				if (response.status === 200 || response.statusText === 'OK') {
					...
					if (
						stipeObject[0].hasOwnProperty('subscriptions') === false
					) {
						let object = exports.licenseSave(null, 'NONE');
						console.log('error');
						resolve(object);
						...
						if (stipeObject[0].subscriptions.total_count != 0) {
							const status =
								stipeObject[0].subscriptions.data[0].status;
							const endTime =
								stipeObject[0].subscriptions.data[0].trial_end;
